...
private:
	...
	void handle_command_ack(const mavlink::mavlink_message_t *msg, mavlink::common::msg::COMMAND_ACK &ack)
	{
		...
		for (auto &tr : ack_waiting_list) {
			if (tr.expected_command == ack.command) {
				...
				tr.result_param1=ack.progress;
				tr.result_param2=ack.result_param2;
				tr.ack.notify_all();
				return;
			}
		}
	...
	bool send_command_long_and_wait(bool broadcast,
		...
		float param7,
		unsigned char &success, uint8_t &result, uint8_t &result_param1, int32_t &result_param2)
	{
		...
		if (is_ack_required) {
			...
			result_param1 = ack_it->result_param1;
			result_param2 = ack_it->result_param2;
		...
		mavros_msgs::CommandLong::Response &res)
	{
		return send_command_long_and_wait(req.broadcast,
			...
			req.param7,
			res.success, res.result, res.result_param1,res.result_param2);
	}
